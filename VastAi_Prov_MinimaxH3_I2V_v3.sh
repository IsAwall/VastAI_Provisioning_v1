...
# THE LADDER
#
#   DRAFT  -> Turbo LoRA, 0.4 MP, 4 steps, SageAttention on, SILENT.
#             Iterate prompt and composition here.
...
#
#   TURBO IS A SILENT DRAFT TIER, NOT A FAST FINAL. It is a step-distillation
#   LoRA: it trains the model to take much larger jumps along the flow
#   trajectory so 4 steps land where ~20 would. That is a fidelity trade by
#   construction. It also produces unreliable audio, and because H3 runs joint
#   video-audio attention, broken audio conditioning propagates back into the
#   video as motion-timing and boundary artifacts. Draft silent, deliberately.
#
...
#   4-step schedule rather than testing anything. Shift work belongs on the
#   base model.
#
...
# of 25 is worth a lot when you are still deciding what the shot is. The
# second entry is the author's reference workflow.
...
NOT IN THIS BUILD
  ...
  H3-Regenerate-2K and H3-Context-IR remain API-only regardless. SeedVR2 is
  your substitute for the first; there is no substitute for the second beyond
  writing better prompts yourself.
