...
# WORKFLOW INSTALL
#
# Cloning a node pack puts its example workflows on disk, but ComfyUI's
# workflow sidebar ONLY lists files under user/default/workflows/. So they are
# invisible until copied. This walks every installed custom node, finds its
# example workflow folder, and copies the JSONs into a per-pack subfolder --
# which is also how they end up grouped rather than dumped in one flat list.
#
# Existing files are NEVER overwritten, so anything you edit and save in
# ComfyUI survives a reboot. To force a refresh of the stock copies:
#     rm -rf /workspace/ComfyUI/user/default/workflows/TTS-Audio-Suite
# and reboot.
#
# Filenames from this pack contain emojis and spaces, hence the -print0 /
# read -d '' handling rather than a bare for-loop over a glob.
# ===========================================================================
WF_ROOT="${COMFY}/user/default/workflows"

install_workflows() {
    local pack pack_name src_dir dest base copied skipped total_copied=0
    mkdir -p "$WF_ROOT" || { echo "[workflows] cannot create $WF_ROOT"; return 0; }

    for pack in "${NODES_DIR}"/*/; do
        pack_name="$(basename "$pack")"
        src_dir=""
        # Packs are inconsistent about the folder name; check both.
        for candidate in "example_workflows" "workflows" "examples"; do
            if [[ -d "${pack}${candidate}" ]] \
               && compgen -G "${pack}${candidate}/*.json" >/dev/null 2>&1; then
                src_dir="${pack}${candidate}"; break
            fi
        done
        [[ -n "$src_dir" ]] || continue

        dest="${WF_ROOT}/${pack_name}"
        mkdir -p "$dest"
        copied=0; skipped=0
        while IFS= read -r -d '' src; do
            ...
            if cp "$src" "${dest}/${base}" 2>/dev/null; then
                copied=$((copied + 1))
            else
                echo "[workflows] copy failed: ${base}"
            fi
        done < <(find "$src_dir" -maxdepth 1 -type f -name '*.json' -print0 2>/dev/null)

        total_copied=$((total_copied + copied))
        echo "[workflows] ${pack_name}: ${copied} new, ${skipped} already present -> workflows/${pack_name}/"
    done

    # Anything you host yourself (e.g. your modified LTX-2.3 IA2V workflow).
    # Add raw URLs to EXTRA_WORKFLOW_URLS below. Percent-encode spaces/emoji.
    if (( ${#EXTRA_WORKFLOW_URLS[@]} > 0 )); then
        mkdir -p "${WF_ROOT}/custom"
        for url in "${EXTRA_WORKFLOW_URLS[@]}"; do
            base="$(basename "${url%%\?*}")"
            # decode %20 etc. so the sidebar shows a readable name
            base="$(printf '%b' "${base//%/\\x}")"
            if [[ -f "${WF_ROOT}/custom/${base}" ]]; then
                echo "[workflows] custom/${base} already present, skipping"; continue
            fi
            if curl -fsSL --connect-timeout 20 --max-time 120 \
                    -o "${WF_ROOT}/custom/${base}" "$url"; then
                # A 404 page saved as .json would silently appear as a broken
                # workflow, so verify it actually parses before keeping it.
                if "$PY" -c "import json,sys; json.load(open(sys.argv[1]))" \
                        "${WF_ROOT}/custom/${base}" 2>/dev/null; then
                    echo "[workflows] fetched custom/${base}"
                    total_copied=$((total_copied + 1))
                else
                    echo "[workflows] DISCARDED custom/${base} -- not valid JSON (bad URL?)"
                    rm -f "${WF_ROOT}/custom/${base}"
                fi
            else
                echo "[workflows] FETCH FAILED: $url"
            fi
        done
    fi

    echo "[workflows] ${total_copied} workflow(s) newly installed into ${WF_ROOT}"
    echo "[workflows] They appear in the ComfyUI sidebar (Workflows tab) after a"
    echo "[workflows] browser refresh -- no ComfyUI restart needed."
    return 0
}

# ---------------------------------------------------------------------------
# Your own hosted workflows -- add raw URLs here.
# Example (note the percent-encoding for spaces and emoji):
#   "https://raw.githubusercontent.com/<you>/<repo>/main/LTX23_IA2V_CustomAudio_v2.json"
# ---------------------------------------------------------------------------
EXTRA_WORKFLOW_URLS=(
    # "https://example.com/path/to/my_workflow.json"
)
...
cat <<'CRIB'
=================== HOW TO USE ===================
[vc] Workflows are now installed into the SIDEBAR automatically (see the
[vc] WORKFLOWS section above). Open the Workflows tab in ComfyUI, folder
[vc] "TTS-Audio-Suite", and load:
[vc]   "Unified <emoji> Voice Changer - RVC X ChatterBox"
[vc] It has both engines pre-wired so you can A/B them on one source clip.
[vc] If you don't see it, hard-refresh the browser (Ctrl+Shift+R).
[vc]
