...
#
# -----------------------------------------------------------------------------
# WHAT CHANGED IN v6  (updates actually happen)
#
#   v5 never updated anything after the first boot. Two independent causes:
#
#     (a) install_node()'s `git pull --ff-only` was COMMENTED OUT. Any node that
#         already existed on the persistent volume was logged "present" and then
#         left alone forever. Every reboot after the first was a no-op for node
#         code, which is why nodes had to be updated by hand.
#
#     (b) ComfyUI core was never updated at all. v5 only ever installed custom
#         nodes and models; nothing in it touched $COMFY.
#
#   Uncommenting the pull would not have been enough. A bare `git pull` fails or
#   silently no-ops in exactly the states these checkouts end up in:
#
#     - DETACHED HEAD. ai-dock images commonly pin core to a commit. `git pull`
#       on a detached HEAD exits 0 and does nothing, so it looks like it worked.
#     - SHALLOW CLONE. Cannot reach arbitrary commits; pins are unresolvable and
#       history-based merges fail.
#     - DIRTY WORKTREE. `pull` aborts, and the loop swallows the error.
#     - origin/HEAD UNSET. Default-branch lookup returns the literal string
#       "(unknown)", which is then checked out and fails.
#
#   v6 adds git_sync(), which handles all four and reports whether HEAD actually
#   moved. That return value drives the two follow-on fixes:
#
#     - CORE REQUIREMENTS ARE REINSTALLED WHEN CORE MOVES. The web UI ships as a
#       pip package (comfyui-frontend-package) pinned in core's requirements.txt.
#       Pulling core without a pip install leaves you on the old frontend --
#       the usual reason an "updated" ComfyUI still looks and behaves stale.
#     - OPTIONAL SERVICE RESTART. ai-dock brings services up around provisioning
#       rather than strictly after it, so code updated here is on disk but not in
#       the running process. If anything changed and comfyui is already running,
#       v6 bounces it. RESTART_COMFY_ON_UPDATE=0 to disable.
#
#   Going the other way, node dependency installs are now CONDITIONAL. v5 ran pip
#   against every node's requirements.txt on every single boot -- minutes of wall
#   clock, and a chance to re-resolve a working environment into a broken one.
#   v6 hashes requirements.txt and only reinstalls when it changed, when the node
#   was freshly cloned, or when FORCE_DEPS=1.
#
#   NODES entries now accept an optional "|<commit-or-tag>" pin. Empty pin (the
#   default) tracks the default branch. Pinning and auto-updating are mutually
#   exclusive by definition -- pick per node.
#
#   Everything else is v5 unchanged: hf_xet Xet-native transport, HF caches under
#   $WORKSPACE, size-verified skip logic, disk pre-flight, aria2 for non-HF URLs.
#
# -----------------------------------------------------------------------------
...
#   COMFY_PIN=<sha>             pin ComfyUI core instead of tracking its branch
...
# ===========================================================================
# RESTART IF CODE CHANGED  (new in v6)
# ===========================================================================
